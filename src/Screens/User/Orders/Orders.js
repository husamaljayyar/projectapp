...
import { ListBox, RowCell, StyledRow, DivContanir, } from "./Orders.Styles";
    ...
    <InnerSection style={{ margin: "44px 0 60px", alignItems: "flex-start" }}>
      ...
      <FlexColumn  style={{paddingTop: '20px'}}>
        <ListBox>  
          ...
          <StyledRow as={"div"} style={{background: "#fcac5e", fontWeight: 700}}>
          ...
          {orders.map((item) => (
            <StyledRow to={`/order/${item._id}`} key={item._id}>
              ...
              <RowCell>{state.userDetails.user.name}</RowCell>
